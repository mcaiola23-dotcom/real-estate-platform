...
def verify_final():
    ...
    try:
        print("\nVerifying Neighborhood Boundaries (Final Check)...")
        ...
        query = text("""
                ...
                FROM (
                    SELECT jsonb_build_object(
                        ...
                        'geometry', ST_AsGeoJSON(COALESCE(ST_SimplifyPreserveTopology(n.boundary, 0.0001), n.boundary))::jsonb
                    ) AS feature
        ...
        if result and result[0]:
            ...
            if features is None:
                ...
                if null_geoms > 0:
                    print(f"❌ ERROR: Still found {null_geoms} features with NULL geometry.")
                else:
