...
def verify_response():
    ...
    try:
        print("\nVerifying Neighborhood Boundaries Response...")
        
        # Test unfiltered query (the one used for main view usually)
        query = text("""
                ...
                FROM (
                    SELECT jsonb_build_object(
                        ...
                        'geometry', ST_AsGeoJSON(ST_Simplify(n.boundary, 0.0001))::jsonb
                    ) AS feature
        ...
        if result and result[0]:
            ...
            if features is None:
                ...
                if len(features) > 0:
                    print(f"Sample Feature Properties: {features[0]['properties']}")
                    if features[0]['geometry'] is None:
                        print("❌ ERROR: Feature 0 geometry is None")
                    else:
                        print("✅ Feature 0 geometry exists")
        else:
